...
# TAB 2: Train Tokenizer
with tab2:
    st.header("🔧 Entraîner le Tokenizer BPE")
    
    if not st.session_state.shakespeare_text:
        st.warning("⚠️ Veuillez d'abord charger le dataset Shakespeare depuis l'onglet 'Load Dataset'")
    else:
        st.info("💡 Le Byte Pair Encoding (BPE) est un algorithme de compression qui identifie et fusionne les paires de caractères les plus fréquentes.")
        
        col1, col2 = st.columns([2, 1])
        
        with col1:
            if not st.session_state.tokenizer_trained:
                if st.button("🚀 Lancer l'entraînement BPE", type="primary", use_container_width=True):
                    with st.spinner("Entraînement en cours..."):
                        ...
                        status_text.text("Étape 1/3: Preprocessing du texte...")
                        progress_bar.progress(33)
                        clean_text = preprocess_for_bpe_training(st.session_state.shakespeare_text)
                        time.sleep(0.5)
                        
                        # Étape 2: Training
                        status_text.text("Étape 2/3: Entraînement du tokenizer BPE...")
                        progress_bar.progress(66)
                        tokenizer = train_bpe_tokenizer(clean_text, vocab_size)
                        st.session_state.tokenizer = tokenizer
                        time.sleep(0.5)
                        
                        # Étape 3: Encoding du corpus
                        status_text.text("Étape 3/3: Encodage du corpus...")
                        progress_bar.progress(100)
                        encoded = torch.tensor(tokenizer.encode(clean_text).ids, dtype=torch.long)
                        
                        # Split train/val
                        n = int(0.9 * len(encoded))
                        st.session_state.train_data = encoded[:n]
                        st.session_state.val_data = encoded[n:]
                        
                        st.session_state.tokenizer_trained = True
                        status_text.empty()
                        progress_bar.empty()
                        
                        st.success("✅ Tokenizer entraîné avec succès!")
                        st.balloons()
            else:
                st.success("✅ Tokenizer déjà entraîné!")
        
        if st.session_state.tokenizer_trained:
            st.markdown("### 📊 Résultats de l'entraînement")
            
            col1, col2, col3, col4 = st.columns(4)
            
            with col1:
                st.metric("Vocabulary Size", f"{st.session_state.tokenizer.get_vocab_size():,}")
            with col2:
                st.metric("Train Tokens", f"{len(st.session_state.train_data):,}")
            with col3:
                st.metric("Val Tokens", f"{len(st.session_state.val_data):,}")
            with col4:
                compression = len(st.session_state.train_data) / st.session_state.data_stats['total_chars'] * 100
                st.metric("Compression", f"{compression:.1f}%")
            
            # Test de tokenization
            st.markdown("### 🧪 Tester le Tokenizer")
            test_text = st.text_input("Entrez un texte à tokeniser:", 
                                      "First Citizen: Before we proceed any further, hear me speak.")
            
            if test_text:
                # Preprocessing
                clean_test = preprocess_for_bpe_training(test_text)
                encoded = st.session_state.tokenizer.encode(clean_test)
                
                col1, col2 = st.columns(2)
                
                with col1:
                    st.markdown("**Tokens:**")
                    tokens_display = " | ".join([f"`{t}`" for t in encoded.tokens])
                ...
                # Décodage
                decoded = st.session_state.tokenizer.decode(encoded.ids)
                st.markdown("**Texte décodé:**")
                st.info(decoded)
